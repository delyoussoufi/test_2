...
from flaskapp.structures.structures import Search, SearchResult, QueryOperators, ColumnOperator, SearchDefault
...
class BaseModel:
    ...
    @classmethod
    def from_dict(cls: Type[B], dto: dict) -> B:
        """
    ...
    @classmethod
    def bulk_save(cls, models: List[db.Model]) -> bool:
        """
    ...
    @classmethod
    def bulk_delete(cls, filters=None) -> bool:
        """
    ...
    def _construct_filter(cls, search: Union[Search, SearchDefault]):
        ...
        for column_name in column_names:  # accepts multiple columns split by ,
            ...
            if search_column is None:
                raise AppException(f"The column {column_name} you are trying to search at {cls.__name__} "
                                   f"doesn't exists.")
            op = QueryOperators(op[0].strip()) if op else search.Operator
            search_columns.append(ColumnOperator(search_column, op))

        if type(search.SearchValue) is str:
            find_values = [v.strip() for v in search.SearchValue.split(",") if type(v) is str]
        else:
    ...
    @classmethod
    def search(cls: Type[B], search) -> SearchResult[B]:
        """
    ...
    @classmethod
    def find_by_id(cls: Type[B], *entity_ids) -> Optional[B]:
        """
        Find by id.
    ...
    @classmethod
    def pagination(cls: Type[B], per_page, page) -> List[B]:
        """
    ...
    @classmethod
    def get_all(cls: Type[B], order_by=None) -> List[B]:
        """
    ...
    @classmethod
    def find_by(cls: Type[B], get_first=True,
                order_by: Union[Column, InstrumentedAttribute] = None, **kwarg) -> Union[None, B, List[B]]:
        """
        Find by a specific column name.
        ...
        :return: The entity when get_first=True, and it exists or a list of entity when get_first=False and exists.
            None, otherwise.
        """
        # Validate class before query
        cls.__class_validation()
    ...
    @classmethod
    def find_by_filter(cls: Type[B],  filters=None, order_by=None, **kwargs) -> Union[List[B], Pagination]:
        """
        Find by the given filters. To paginate it both kwargs, page and per_page must be provided. Otherwise,
        it will return all the found entities.
